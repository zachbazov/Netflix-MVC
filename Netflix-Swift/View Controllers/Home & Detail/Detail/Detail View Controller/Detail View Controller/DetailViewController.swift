...
final class DetailViewController: PrototypeViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        flowLayout = ComputableFlowLayout(.detail, .vertical)
        ...
        guard let section = detailViewModel.section else { return }
        dataSet = DetailDataSet(section, detailViewController: self)
        snapshot = DetailSnapshot(dataSet, homeViewController)
        collectionView.delegate = snapshot
        collectionView.dataSource = snapshot
        collectionView.prefetchDataSource = snapshot
        guard let detailViewModelDelegate = detailViewModel.delegate else { return }
        WeakInjector.shared.inject(detailViewModel, with: self)
        detailViewModelDelegate.detailViewModelDidValidateScrollViewBounds(scrollView, with: contentViewHeightConstraint)
    ...
    deinit {
        guard let mediaPlayerViewDelegate = detailPreviewView.mediaPlayerView.delegate else { return }
