...
protocol MediaDisplayViewDelegate: AnyObject {
    ...
    func mediaDisplayView(_ mediaDisplayView: MediaDisplayView,
                          willDisplay media: MediaViewModel)
    
    func mediaDisplayView(_ mediaDisplay: MediaDisplayView,
                          genreForItem media: MediaViewModel)
}
...
final class MediaDisplayView: UIView, Nibable {
    ...
    weak var homeViewController: HomeViewController! = nil {
        didSet {
            ...
            guard mediaViewModel != homeViewController.homeViewModel.displayMedia! else {
                return
            }
...
extension MediaDisplayView: MediaDisplayViewDelegate {
    ...
    func mediaDisplayView(_ mediaDisplayView: MediaDisplayView, willDisplay media: MediaViewModel) {
        guard
            let covers = media.covers,
            let cover = covers.randomElement(),
            let logo = media.logo,
        ...
        URLService.shared.downloadImage(coverURL, for: identifier) { [weak self] image in
            guard
                let self = self,
        ...
        URLService.shared.downloadImage(logoURL, for: logoIdentifier) { [weak logoImageView] image in
    ...
    func mediaDisplayView(_ mediaDisplay: MediaDisplayView, genreForItem media: MediaViewModel) {
        ...
        for attributedGenre in attributedGenres {
            ...
            let separatorAttributes = [NSAttributedString.Key.font: UIFont.systemFont(ofSize: separatorSize,
                                                                                      weight: .heavy)]
            
            let attributedSeparator = NSAttributedString(string: symbol, attributes: separatorAttributes)
            
            if attributedGenre == attributedGenres.last {
                continue
            }
        ...
        guard let label = mediaDisplay.genresLabel else {
            return
        }
