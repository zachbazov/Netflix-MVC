...
final class HomeViewModel: NSObject {
    ...
    weak var homeViewController: HomeViewController! = nil {
        didSet {
            guard let tableViewSnapshotDelegate = tableViewSnapshotDelegate else {
                return
            }
...
extension HomeViewModel: HomeViewModelDelegate {
    ...
    func filter(_ sections: [SectionViewModel], at index: Int) {
        currentSnapshot == .tvShows ? {
            sections[index].media = sections.first!.media.filter {
    ...
    func filter(_ sections: [SectionViewModel]) {
        for i in SectionIndices.allCases {
            ...
            case .display,
                ...
                currentSnapshot == .tvShows ? {
                    sections[i.rawValue].media = sections.first!.media
                }() : {
    ...
    func randomObject(at section: SectionViewModel) -> MediaViewModel? {
        guard let media = currentSnapshot == .tvShows ? section.media.randomElement() : section.movies.randomElement() else {
            return nil
        }
...
extension HomeViewModel: TableViewSnapshotDelegate {
    ...
    func tableViewDidInstantiateSnapshot(_ state: TableViewSnapshot.State) {
        ...
        guard
            ...
            let listKey: ListKey = state == .tvShows ? .tvShows : .movies
        else {
            return
        }
        ...
        snapshot = .init(state, with: homeViewController)
        
        
        homeViewController.tableView.reloadData()
        
        
        myList = .init(forKey: listKey)
